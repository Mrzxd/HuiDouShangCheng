...
@interface EditAddressController ()  <UIPickerViewDelegate,UIPickerViewDataSource,UITextFieldDelegate>
...
- (void)viewDidLoad {
    ...
    _sex = _addressModel.sex;
    _isDefault = 1;
    _selectRow1 = _selectRow2 = _selectRow3  = 0;
    _selectIdArray = @[_addressModel.sheng_id,_addressModel.city_id,_addressModel.area_id].mutableCopy;
    _textFieldArray = [NSMutableArray new];
    
    NSArray *array1 = @[@"联系人：",@"手机号：",@"详细地址："];
    NSArray *array2 = @[@"请输入姓名",@"请输入手机号码",@"详细地址（例如：3号楼5层501室）"];
    [self.rightButtons setTitle:@"删除" forState:UIControlStateNormal];
    ...
    for (NSInteger i = 0; i < 3; i ++) {
        ...
        _textField.text = @[NoneNull(_addressModel.name),NoneNull(_addressModel.phone),NoneNull(_addressModel.address)][i];
    ...
    for (NSInteger i = 0; i < 2; i ++) {
        ...
        if ([_addressModel.sex containsString:@"女"]) {
            if (i == 0) {
    ...
    _addressContentLabel.text = NoneNull(_addressModel.sca_name);
    ...
    UISwitch *switchs = [[UISwitch alloc] initWithFrame:AutoFrame(309,278, 30, 18)];
    switchs.on = [_addressModel.is_default intValue];
    switchs.onTintColor = RGBHex(0xE70422);
    switchs.thumbTintColor = [UIColor whiteColor];
    switchs.tintColor = [UIColor blueColor];
    switchs.layer.cornerRadius = 14*ScalePpth;
    switchs.layer.masksToBounds = YES;
    switchs.clipsToBounds = YES;
    switchs.backgroundColor = UIColor.lightGrayColor;
    [switchs addTarget:self action:@selector(valueChangedMethod:) forControlEvents:UIControlEventValueChanged];
...
- (void)connectToTheNetwork {
    ...
    [ZXD_NetWorking postWithUrl:[rootUrl stringByAppendingString:@"/api/user/updateAddress"] params:@{
                                                                                                      ...
                                                                                                      @"address_id":_addressModel.idName
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    _addressContentLabel.text = [NSString stringWithFormat:@"%@,%@,%@",_dataArray[_selectRow1][@"title"],_dataArray[_selectRow1][@"son"][_selectRow2][@"title"],[_dataArray[_selectRow1][@"son"][_selectRow2][@"son"] count] ? _dataArray[_selectRow1][@"son"][_selectRow2][@"son"][_selectRow3][@"title"] :@""];
    _selectIdArray = @[_dataArray[_selectRow1][@"id"],_dataArray[_selectRow1][@"son"][_selectRow2][@"id"],[_dataArray[_selectRow1][@"son"][_selectRow2][@"son"] count] ?_dataArray[_selectRow1][@"son"][_selectRow2][@"son"][_selectRow3][@"id"]:@""].mutableCopy;
    [pickerView reloadAllComponents];
}

- (void)rightButtonAction:(UIButton *)button {
    WeakSelf;
    [ZXD_NetWorking postWithUrl:[rootUrl stringByAppendingString:@"/api/user/delMyAddress"] params:@{@"address_id":_addressModel.idName} success:^(id  _Nonnull response) {
