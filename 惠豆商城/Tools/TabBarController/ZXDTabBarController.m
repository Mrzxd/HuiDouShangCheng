...
@interface ZXDTabBarController () <AxcAE_TabBarDelegate>

@end

@implementation ZXDTabBarController
...
//    HomePageController *homeVC = [[HomePageController alloc] init];
    CommodityClassificationController *wallteVc = [CommodityClassificationController new];
    ShoppingCartController *pController  = [ShoppingCartController new];
    MineController *four  = [MineController new];
    NSArray <NSDictionary *>*VCArray =
    @[@{@"vc":vc,@"normalImg":@"首页",@"selectImg":@"首页填充",@"itemTitle":@"首页"},
      @{@"vc":wallteVc,@"normalImg":@"分类",@"selectImg":@"分类填充",@"itemTitle":@"分类"},
      @{@"vc":pController,@"normalImg":@"购物车",@"selectImg":@"购物车填充",@"itemTitle":@"购物车"},
      @{@"vc":four,@"normalImg":@"我的",@"selectImg":@"我的填充",@"itemTitle":@"我的"},
    ];
    // 1.遍历这个集合
    // 1.1 设置一个保存构造器的数组
    ...
    [VCArray enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        model.selectColor = RGBHex(0xFF5F56);
       model.normalColor = RGBHex(0xBBBBBB);
